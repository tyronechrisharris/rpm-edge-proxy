...
class NetworkTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_rpm_broadcast_to_two_clients_and_discards_writes(self):
        ...
        try:
            ...
            await asyncio.sleep(0.05)
            self.assertEqual(len(upstream_writers), 1, "all clients must share one RPM connection")
            upstream_writers[0].write(b"scan-")
            upstream_writers[0].write(b"data")
            ...
            self.assertEqual(stats.bytes_from_upstream, 9)
            self.assertEqual(stats.bytes_to_clients, 18)
